...
def fetch_replacement_words(word, num_of_candidates, embedding):
    try:
...
def change_words(text, num_of_candidates= 2):
    text_list, mask_list = words2swap(text)
    embedding = WordEmbedding.counterfitted_GLOVE_embedding()
    word_dict = {}
    for word in text_list:
        candidate_word_list = fetch_replacement_words(word, num_of_candidates, embedding)
